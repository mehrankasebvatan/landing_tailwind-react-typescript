    ...
    return (
        <div className="flex items-center gap-1">
            {[...Array(5)].map((_, i) => (
                ...
                ${i < Math.floor(rating) ? 'text-amber-400 fill-amber-400' : 'text-text-muted'}`} key={i}/>
            ))}
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {courses.map((course, i) => (
                        <motion.div
                            ...
                            className={`group relative flex flex-col rounded-2xl bg-surface border  border-surface-border
                             ${course.borderColor} transition-all duration-300 hover:bg-surface-light hover:shadow-xl 
                             hover:shadow-dark-900/50 hover:-translate-y-1 overflow-hidden`}
                        >
                            {course.popular && (
                                <Badge className="absolute top-4 right-4 z-10" variant='primary'>🔥 Most Popular</Badge>
                            )}

                            <div className={`relative h-40 bg-linear-to-br ${course.gradient} flex items-center 
                            justify-center`}>
                            ...
                            <div className="flex flex-col flex-1 p-6">
                                <div className="flex flex-wrap gap-2 mb-3">
                                    {course.tags.map((tag) => (
                                        <span className='text-xs font-medium px-2.5 py-1 rounded-md bg-dark-700
                                        text-text-secondary' key={tag}>{tag}</span>
                                    ))}
                                </div>
                                <h3 className="text-lg font-semibold text-text-primary mb-2
                                group-hover:text-primary-light transition-colors">{course.title}</h3>
                                <p className="text-sm text-text-secondary leading-relaxed mb-4 flex-1">
                                    {course.description}</p>

                                <div className="flex items-center gap-4 text-xs text-text-muted mb-4">
                                    <span className="inline-flex items-center gap-1"><Clock
                                        className="w-3.5 h-3.5"/>{course.duration}</span>
                                    <span className="inline-flex items-center gap-1"><BarChart3
                                        className="w-3.5 h-3.5"/>{course.level}</span>
                                    <span className="inline-flex items-center gap-1"><Users
                                        className="w-3.5 h-3.5"/>{course.students}</span>
                                </div>

                                <StarRating rating={course.rating}/>

                                <div className="mt-5 pt-5 border-t border-surface-border flex items-center
                                justify-between">
                                    <div className="flex items-baseline gap-2">
                                        <span className="text-2xl font-bold text-text-primary">{course.price}</span>
                                        <span
                                            className="text-sm text-text-muted line-through">{course.originalPrice}</span>
                                    </div>
                                    <Button variant="primary" size="sm">Enroll <ArrowRight
