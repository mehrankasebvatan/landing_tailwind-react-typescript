    ...
    return (
        <footer className="relative border-t border-surface-border bg-dark-800">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="py-16 grid grid-cols-2 md:grid-cols-6 gap-8 lg:gap-12">
                    <div className="col-span-2">
                        <a href="#home" className="flex items-center  gap-2.5 mb-5">
                            <div
                                className="flex items-center justify-center w-9 h-9 rounded-lg bg-linear-to-br from-primary to-accent-blue">
                                {" "}
                        ...
                        <div className="flex gap-3">
                            {socialLinks.map((socialLink) => (
                                <a className="w-10 h-10 rounded-xl bg-surface border border-surface-border flex
                                items-center justify-center text-text-muted hover:text-primary-light
                                hover:border-primary/30hover:bg-primary/10 transition-all duration-200">
                                    {" "}
                    ...
                    {Object.entries(footerLinks).map(([category, links]) => (
                        <div key={category}>
                            ...
                            <ul className="space-y-3">
                                {links.map((link) => (
                                    <li key={link.name}>
                                        <a className="text-sm text-text-muted hover:text-primary-light
                                        transition-colors duration-200" href={link.href}>{link.name}</a>
                                    </li>
                                ))}
                            </ul>
                        </div>
                ...
                <div className="flex flex-col sm:flex-row gap-4 items-center justify-between py-6 border-t border-surface-border">
